...
def get_training_set(c_len, bch_size):  # 从文本中随机选取一段进行训练
    ...
    for bi in range(bch_size):
        ...
        chunk = input_file[start_index:
                           end_index]  # chunk取得的长度是c_len + 1，比x[bi]和y[bi]都多1
...
class LstmNet(t.nn.Module):  # Lstm 网络
    def __init__(self, input_size, h_size, n_layers, num_classes):
        ...
        self.lstm = t.nn.LSTM(
            h_size, h_size, n_layers, batch_first=True)  # LSTM层
...
def generate(decoder,
    ...
    for p in range(generate_len):
        ...
        inp = t.autograd.Variable(
            char_to_tensor(predicted_char).unsqueeze(0))  # 更新预测输入值
...
model = LstmNet(num_printable, hidden_size, num_layers,
                num_printable)  # LSTM模型
...
for epoch in range(1, num_epochs + 1):
    ...
    loss = 0
    x_tensor, y_tensor = get_training_set(sentence_length,
                                          batch_size)  # 获取随机文本进行训练
    ...
    if epoch % progress_every == 0:
        ...
        print(
            generate(
                model, all_printable, seed_string='The', generate_len=100))
